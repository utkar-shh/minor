...
def plot_dashboard(df, ticker, sentiment_score, prediction, confidence, predicted_price, current_price):
    ...
    # Updated title to include the target price
    fig.suptitle(f'{ticker} Real-Time Market Analysis\nTarget: ${predicted_price:.2f} | Confidence: {confidence:.2f}% | Live NLP Sentiment: {sentiment_score:.2f}', 
    ...
    # NEW: Plot the predicted future price as a target line
...
# --- SWAP 1: UPDATED TRAINING DATA & TARGET ---
    print(f"\n[*] Fetching 4-year market data for {ticker} to train background models...")
   
    # Use period="4y" instead of hardcoded past dates so it stays current forever
    ...
    # Target 1: Direction (Classification)
    df['Target_Direction'] = (df['Close'].shift(-1) > df['Close']).astype(int)
    
    # Target 2: FUTURE RETURN instead of Future Price (Regression)
    # This mathematically solves the extrapolation problem
    df['Target_Return'] = df['Close'].pct_change().shift(-1) 
    
    df.dropna(inplace=True)

    print("[*] Training XGBoost Prediction Models (Classifier & Regressor)...")
    ...
    # Train Classifier for Confidence/Direction
    classifier = xgb.XGBClassifier(n_estimators=150, max_depth=3, learning_rate=0.05, random_state=42)
    classifier.fit(df[features], df['Target_Direction'])

# --- SWAP 2: UPDATED PREDICTION MATH ---
    # Train Regressor for Percentage Return (Change 'Target_Price' to 'Target_Return')
    regressor = xgb.XGBRegressor(n_estimators=150, max_depth=3, learning_rate=0.05, random_state=42)
    regressor.fit(df[features], df['Target_Return'])
    
    # ... (Keep the live data fetching code exactly the same) ...
    
    # Make Predictions
    prediction = classifier.predict(today_data[features])[0]
    probability = classifier.predict_proba(today_data[features])[0]
    confidence = probability[1]*100 if prediction == 1 else probability[0]*100
    
    # Predict the percentage move, then calculate the price
    predicted_return = regressor.predict(today_data[features])[0]
    ...
    predicted_price = regressor.predict(today_data[features])[0]

    # Calculate projected percentage move
    projected_move = ((predicted_price - current_price) / current_price) * 100
    ...
    plot_dashboard(live_df, ticker, live_sentiment_score, prediction, confidence, predicted_price, current_price)
